...
@section('title', 'Nouvelle Prime')

@section('content')
<div class="p-6 animate-fadeIn">
    <div class="mb-6">
        <a href="{{ route('primes.index') }}" class="inline-flex items-center gap-2 text-sm text-gray-600 hover:text-gray-900 transition-colors">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-6 border-b border-gray-100">
            <h1 class="text-2xl font-bold text-gray-900">Nouvelle Prime</h1>
            <p class="text-sm text-gray-500 mt-1">Ajoutez une prime pour un employé</p>
        </div>

        <form method="POST" action="{{ route('primes.store') }}" class="p-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="type_prime" required placeholder="Ex: Prime rendement, Prime transport" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-primary focus:border-transparent">
                    @error('type_prime')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="flex gap-3 mt-6">
                ...
                <a href="{{ route('primes.index') }}" class="px-6 py-2 bg-gray-100 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-200 transition-colors">
                    Annuler
                </a>
            </div>
        </form>
